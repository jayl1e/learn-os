...
impl PhysPageNum {
    pub fn bytes_mut(&self)->&'static mut [u8]{
        ...
        unsafe {
            slice::from_raw_parts_mut(start as *mut u8, PAGE_SIZE)
        }
    }
    pub fn pte_array_mut(self)->&'static mut [PageTableEntry]{
        let pa: usize = PhysAddress::from(self).into();
        unsafe {
            slice::from_raw_parts_mut(pa as *mut PageTableEntry, PAGE_SIZE/size_of::<PageTableEntry>())
        }
    }
...
impl VPNRange {
    pub fn new(l: VirtPageNum, r:VirtPageNum)->Self{Self{l,r}}
}
...
impl IntoIterator for &VPNRange {
    ...
    fn into_iter(self) -> Self::IntoIter {
        VPNRangeIter{c:self.l,r:self.r} 
    }
}
