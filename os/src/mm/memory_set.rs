...
use crate::{config::{KERNEL_STACK_LIMIT, USER_STACK_LIMIT}, mm::address::PhysAddress, println, sync::up::UPSafeCell};

use super::{address::{VPNRange, VirtAddress, VirtPageNum, PAGE_SIZE}, frame_allocator::{frame_new, FrameGuard, MEMORY_END}, page_table::{PTEFlags, PageTable}};
...
impl MapArea {
    fn new(start: VirtAddress, end: VirtAddress, tp: MapType, perm: MapPermission)->Self{
        Self { vpns: VPNRange::new(start.floor(), end.ceil()), frames: BTreeMap::new(), map_type: tp, map_perm: perm }
    } 
...
impl MemorySet {
    ...
    fn map_trampoline(&mut self){
        ...
        self.page_table.map(VirtAddress(TRAMPOLINE).into(), PhysAddress(strampoline as usize).into(),PTEFlags::X|PTEFlags::R);
    }
    pub fn new_app_from_elf(elf: &[u8])->(Self, usize, usize){
        ...
        for header in elf.program_iter(){
            if header.get_type().unwrap() == xmas_elf::program::Type::Load{
                ...
                let hf = header.flags();
                if hf.is_read(){flag |= MapPermission::R;}
                if hf.is_write(){flag |= MapPermission::W;}
                if hf.is_execute(){flag |= MapPermission::X;}
                let area = MapArea::new(start_va, end_va, MapType::Framed, flag);
                max_end_vpn = max(max_end_vpn, area.vpns.r);
                let data = &elf.input[header.offset() as usize .. (header.offset()+header.file_size()) as usize];
        ...
        ms.push(MapArea::new(stack_bottom, stack_top, MapType::Framed, MapPermission::U|MapPermission::R|MapPermission::W), None);
        // map the trap context page
        ms.push(MapArea::new(TRAP_CONTEXT.into(), TRAMPOLINE.into(), MapType::Framed, MapPermission::R|MapPermission::W), None);
...
fn new_kernel_map()->MemorySet{
    ...
    ms.push(MapArea::new((stext as usize).into(), (etext as usize).into(), MapType::Identical, MapPermission::X|MapPermission::R), None);
    debug!("map rodata");
    ms.push(MapArea::new((srodata as usize).into(), (erodata as usize).into(), MapType::Identical, MapPermission::R), None);
    debug!("map data");
    ms.push(MapArea::new((sdata as usize).into(), (edata as usize).into(), MapType::Identical, MapPermission::R|MapPermission::W), None);
    debug!("map bss");
    ms.push(MapArea::new((sbss_with_stack as usize).into(), (ebss as usize).into(), MapType::Identical, MapPermission::R|MapPermission::W), None);
    debug!("map endkernel");
    ms.push(MapArea::new((ekernel as usize).into(), (MEMORY_END as usize).into(), MapType::Identical, MapPermission::R|MapPermission::W), None);
    ms
}
...
fn test_kernel_remap(){
    ...
    let text_seg = kernel.page_table.translate(VirtAddress(stext as usize ).into()).unwrap();
    assert_eq!(true, text_seg.executable());
    assert_eq!(true, text_seg.readable());
    assert_eq!(false, text_seg.writable());
    let rodata = kernel.page_table.translate(VirtAddress(srodata as usize ).into()).unwrap();
